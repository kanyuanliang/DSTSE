...
class Actor(nn.Module):
	...
	n=22,linear1=[6,4,4,4],linear2=[4,4,2,1],linear3=[4,4,4,4],gnn1=[4,4]):
		...
		last_shape=torch.ones(n,1)
		last_shape[-1]=0
		self.last_shape=last_shape.to(device)
		
		self.A_shape=p_shape+p_shape.transpose(1,0)+torch.eye(n).to(device)
	...
	def forward(self,x):
		...
		p=p1@(p2.transpose(2,1))
		p=torch.tanh(p/4)*4
		p=torch.exp(p)
		p=p*self.p_shape
		p=p/(torch.unsqueeze(torch.sum(p,-1),-1)+1e-15)
		
		b,d,u=torch.abs(b),torch.abs(d),torch.abs(u)
		b,d,u=torch.where(b<500,b,0.01*b+495),torch.where(d<500,d,0.01*d+495),torch.where(u<800,u,0.01*u+792)
		...
		v=torch.where(v>1.05,(v-1.05)*0.01+1.05,v)
