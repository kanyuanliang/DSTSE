...
from inference_model.inference_model_for_dataset1 import *
...
train_data=torch.load('data_of_dataset1/input/flow_data.pt').to(device)
...
actor_path='data_of_dataset1/output/inference_model_for_dataset1_gat.pt'

state_dict = torch.load('data_of_dataset1/output/pretrained_fitting_model_for_dataset1.pt', map_location='cpu')
...
equation=Equation(batch_size,23,5,2,40)
test_equation=Equation(192,23,5,2,40)
...
while True:
	...
	for data in train_loader:
		...
		bd_loss=criterion(b[:,1:],zero)+criterion(d[:,:-1],zero)
		u_loss=criterion(torch.squeeze(u2)[:,:-1],differential_u2[:,:-1])
		
		train_epoch_loss=actor_loss+bd_loss+1e6*torch.relu(v_loss-0.005)+5*u_loss
	...
	with torch.no_grad():
		...
		test_loss=actor_loss+bd_loss+1e6*torch.relu(v_loss-0.005)+5*u_loss
		...
		if test_loss<min_test_loss:
			...
			print("")
			torch.save(v,'data_of_dataset1/output/inference_model_for_dataset1_gat_v.pt')
			torch.save(b,'data_of_dataset1/output/inference_model_for_dataset1_gat_b.pt')
			torch.save(d,'data_of_dataset1/output/inference_model_for_dataset1_gat_d.pt')
			torch.save(u,'data_of_dataset1/output/inference_model_for_dataset1_gat_u.pt')
			torch.save(differential_u2,'data_of_dataset1/output/inference_model_for_dataset1_gat_differential_u2.pt')
			torch.save(u2,'data_of_dataset1/output/inference_model_for_dataset1_gat_u2.pt')
			torch.save(net_flow,'data_of_dataset1/output/inference_model_for_dataset1_gat_flow.pt')
			torch.save(differential_flow,'data_of_dataset1/output/inference_model_for_dataset1_gat_differential_flow.pt')
